        ...
        <div className={'container hidden lg:flex justify-between text-white'}>
            ...
            <div className={'flex lg:gap-5 xl:gap-10 py-2.5 items-center font-gilroy font-bold text-base xl:text-lg'}>
                {menu?.links && menu.links.map((e, _uid)=>(
                            <Link href={e.link.linktype === "story" ? "/"+e.link.cached_url : e.link.cached_url} key={_uid} className={clsx(!e.contact && 'hover:text-yellow-active', e.contact && 'border-yellow-active text-yellow-active border rounded-3xl lg:px-2.5 lg:py-2 xl:px-3 xl:py-2.5 hover:bg-yellow-active hover:text-black')}>
                                {e.label}
                            </Link>
                   ))}
            </div>
