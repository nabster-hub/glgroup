...
export async function generateMetadata({ params, params: { locale } }, parent) {
    ...
    const res = await fetchData(`/currency/${params.slug}`, { version: 'draft', language: locale });
    ...
    if (!res) {
        ...
        return {
            ...
            alternates: {
                canonical: './',
                languages: {
                    'ru_RU': 'https://www.glgconsult.com/ru/currency' + data.story.slug,
                    'en_EN': 'https://www.glgconsult.com/en/currency' + data.story.slug,
                },
            },
            openGraph: {
        ...
        <>
            ...
            {data?.story.content.body?.map((blok, index) => (
                <React.Fragment key={blok._uid}>
                    ...
                    {/* Вставляємо додаткові блоки після першого блоку */}
                    {index === 0 && servicesListBlock && (
                        <ServicesListComponent blok={servicesListBlock} />
                    )}
                    {index === 0 && ourRecommendationsBlock && (
